...
func TestDocResource(t *testing.T) {
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			// === Test updating the doc title and body attributes.
			{
				Config: `
					resource "readme_category" "test" {
						title = "Test Category"
					...
					resource "readme_doc" "test" {
						title    = "My Test Doc Updated"
						body     = "This is an updated test body"
						category = readme_category.test.id
						type     = "basic"
					}`,
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestMatchResourceAttr(
						"readme_doc.test",
						"title",
						regexp.MustCompile(`^My Test Doc Updated$`),
					),
					resource.TestMatchResourceAttr(
						"readme_doc.test",
						"body",
						regexp.MustCompile(`^This is an updated test body$`),
					),
				),
			},
...
func TestDocResourceImport(t *testing.T) {
	resource.Test(t, resource.TestCase{
		ProtoV6ProviderFactories: testProtoV6ProviderFactories,
		Steps: []resource.TestStep{
			// === Happy path doc creation test.
